let sqlQueries = {
    ...
    getCustomerReservations: 'SELECT R.id, B.bName, R.pax, R.reservedslot, R.reserveddate, P.promo_code FROM (Reservations R INNER JOIN Branches B ON R.branch_id = B.id) LEFT JOIN Promotions P on R.promo_used = P.promo_code WHERE R.customer_id = $1',
    getOwnerUnconfirmedReservations: 'select r.id as reservation_id, restaurant_id, bname, u.username, pax, reservedslot, reserveddate, promo_code from \n' +
        ...
        '\treservations r on b.id = r.branch_id left join promotions p on promo_used = p.promo_code) inner join users u on u.id = r.customer_id\n' +
